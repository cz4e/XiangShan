...
class FreeList(size: Int, allocWidth: Int, freeWidth: Int, moduleName: String = "")(implicit p: Parameters) extends XSModule
  with HasCircularQueuePtrHelper
{
  ...
  val freeReq = VecInit(remFreeSelMaskVec.map(x => RegNext(x).orR))
  ...
  val numAllocate = PopCount(io.doAllocate) // last requires
