  ...
  with HasPerfEvents
{
  val io = IO(new Bundle() {
    ...
    val lqCanAccept = Output(Vec(LoadPipelineWidth, Bool()))
  ...
  // generate enq mask
  val loadEnqValidMask = io.enq.zip(canEnqueueVec).map(x => x._2 && !x._1.bits.isLoadReplay).zip(selectIndexOH).map(x => Mux(x._1, x._2, 0.U))
  val remLoadEnqValidVec = loadEnqValidMask.map(x => VecInit((0 until LoadPipelineWidth).map(rem => getRemBits(x)(rem))))
  val remEnqSelVec = Seq.tabulate(LoadPipelineWidth)(w => VecInit(remLoadEnqValidVec.map(x => x(w))))
  ...
  val ColdDownThreshold = 12
  require(ColdDownCycles > ColdDownThreshold, "ColdDownCycles must great than ColdDownThreshold!")

  val coldCounter = RegInit(VecInit(List.fill(LoadPipelineWidth)(0.U(log2Up(ColdDownCycles).W))))
  def replayCanFire(i: Int) = coldCounter(i) >= 0.U && coldCounter(i) < ColdDownThreshold.U
  def coldDownNow(i: Int) = coldCounter(i) >= ColdDownThreshold.U
  ...
  for (i <- 0 until LoadPipelineWidth) {
    ...
    when (io.replay(i).fire) {
      ...
      assert(allocated(s2_oldestSel(i).bits), s"why replay an invalid entry ${s2_oldestSel(i).bits} ?\n")
    }
  }
  ...
  // enqueue
  val canAccept = (0 until LoadPipelineWidth).map(w => {
      if (w == 0) {
        !io.lqFull || io.enq(w).bits.isLoadReplay
      } else  {
        PopCount(io.enq.take(w).map(x => x.valid && x.bits.replayInfo.needReplay() && !x.bits.isLoadReplay)) < lqFreeNums || io.enq(w).bits.isLoadReplay
      }
  })

  for ((enq, w) <- io.enq.zipWithIndex) {
    val enqIdx = Mux(enq.bits.isLoadReplay, enq.bits.sleepIndex, OHToUInt(selectIndexOH(w)))
    vaddrModule.io.wen(w) := false.B

    io.lqCanAccept(w) := canAccept(w)

    when (canEnqueueVec(w) && canAccept(w)) {
      assert(!allocated(enqIdx) || enq.bits.isLoadReplay, s"Can not accept more load, check robIdx: ${enq.bits.uop.robIdx}!")
      ...
      // update credit
      val blockCyclesTlbPtr = blockPtrTlb(enqIdx)
      val blockCyclesCachePtr = blockPtrCache(enqIdx)
      val blockCyclesOtherPtr = blockPtrOthers(enqIdx)
      creditUpdate(enqIdx) := Mux(replayInfo.cause(LoadReplayCauses.tlbMiss), blockCyclesTlb(blockCyclesTlbPtr), 
      ...
      when (replayInfo.cause(LoadReplayCauses.tlbMiss)) {
        ...
        blockPtrTlb(enqIdx) := Mux(blockPtrTlb(enqIdx) === 3.U(2.W), blockPtrTlb(enqIdx), blockPtrTlb(enqIdx) + 1.U(2.W))
      }

      // special case: dcache miss
      when (replayInfo.cause(LoadReplayCauses.dcacheMiss)) {
        blockByCacheMiss(enqIdx) := !replayInfo.canForwardFullData && //  dcache miss
        ...
        blockPtrCache(enqIdx) := Mux(blockPtrCache(enqIdx) === 3.U(2.W), blockPtrCache(enqIdx), blockPtrCache(enqIdx) + 1.U(2.W))
      }
  ...
  //  perf cnt
  val enqCount = PopCount(io.enq.map(enq => enq.valid && !enq.bits.isLoadReplay)) 
  val deqCount = PopCount(io.replay.map(_.fire)) 
  val deqBlockCount = PopCount(io.replay.map(r => r.valid && !r.ready))
  val replayTlbMissCount = PopCount(io.enq.map(enq => enq.valid && !enq.bits.isLoadReplay && enq.bits.replayInfo.cause(LoadReplayCauses.tlbMiss)))
  val replayWaitStoreCount = PopCount(io.enq.map(enq => enq.valid && !enq.bits.isLoadReplay && enq.bits.replayInfo.cause(LoadReplayCauses.waitStore)))
  val replaySchedErrorCount = PopCount(io.enq.map(enq => enq.valid && !enq.bits.isLoadReplay && enq.bits.replayInfo.cause(LoadReplayCauses.schedError)))
  val replayRejectEnqCount = PopCount(io.enq.map(enq => enq.valid && !enq.bits.isLoadReplay && enq.bits.replayInfo.cause(LoadReplayCauses.rejectEnq)))
  val replayBankConflictCount = PopCount(io.enq.map(enq => enq.valid && !enq.bits.isLoadReplay && enq.bits.replayInfo.cause(LoadReplayCauses.bankConflict)))
  val replayDCacheReplayCount = PopCount(io.enq.map(enq => enq.valid && !enq.bits.isLoadReplay && enq.bits.replayInfo.cause(LoadReplayCauses.dcacheReplay)))
  val replayForwardFailCount = PopCount(io.enq.map(enq => enq.valid && !enq.bits.isLoadReplay && enq.bits.replayInfo.cause(LoadReplayCauses.forwardFail)))
  val replayDCacheMissCount = PopCount(io.enq.map(enq => enq.valid && !enq.bits.isLoadReplay && enq.bits.replayInfo.cause(LoadReplayCauses.dcacheMiss)))
