  ...
  with HasPerfEvents
{
  ...
  val issued = RegInit(VecInit(List.fill(LoadQueueFlagSize)(false.B))) // inst issued
  ...
  when (isAfter(enqPtrExtNextVec(0), deqPtrExtNext)) {
    enqPtrExtNext := enqPtrExtNextVec
  } .otherwise {
    enqPtrExtNext := VecInit((0 until io.enq.req.length).map(i => deqPtrExtNext + i.U))
  }
  ...
  require(IssPtrMoveStride == CommitWidth, "IssPtrMoveStride must be equal to CommitWidth!")
  ...
  val issPtrExtMove = issPtrExt + lastIssCount
  when (lastCycleRedirect.valid) {
    issPtrExtNext := Mux(isAfter(enqPtrExtNext(0), issPtrExtMove), issPtrExtMove, enqPtrExtNext(0))
  } .otherwise {
    issPtrExtNext := issPtrExtMove
  }
  issPtrExt := RegEnable(next = issPtrExtNext, init = 0.U.asTypeOf(new LqPtr), enable = issPtrExtEna)
  io.ldIssuePtr := issPtrExt // deqPtrExt 
  /**
   * Enqueue at dispatch
   * 
  ...
  (0 until IssPtrMoveStride).map(i => {
    when (issCount > i.U) { // don't update in redirect cycle
  ...
  for   (i <- 0 until LoadPipelineWidth) {
    ...
    when (io.loadIn(i).valid) {
      ...
      issued(loadWbIndex) := io.loadIn(i).bits.mmio ||  // mmio load
                             hasExceptions ||  // has exceptions
                            !needReplay && !io.loadIn(i).bits.tlbMiss && datavalidVal // no need replay and datavalid
    ...
    io.rob.mmio(i) := RegNext(io.loadIn(i).valid && io.loadIn(i).bits.mmio)
